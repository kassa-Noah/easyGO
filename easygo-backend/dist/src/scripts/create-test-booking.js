...
const TRIP_ID = "dc751d5e-d7cd-432b-8d9d-67bf108fda10";
const BOOKING_REFERENCE = "EASYGO-JOURNEY-TEST-001";
const main = async () => {
    ...
    if (!customer) {
        throw new Error(`Customer ${CUSTOMER_EMAIL} was not found`);
    }
    if (customer.role !== "CUSTOMER") {
        throw new Error(`${CUSTOMER_EMAIL} is not a CUSTOMER`);
    }
    ...
    if (!trip) {
        throw new Error(`Trip ${TRIP_ID} was not found`);
    }
...
main()
    .catch((error) => {
    console.error("Unable to create test booking:", error);
    process.exitCode = 1;
})
    .finally(async () => {
    await prisma_1.default.$disconnect();
});
//# sourceMappingURL=create-test-booking.js.map