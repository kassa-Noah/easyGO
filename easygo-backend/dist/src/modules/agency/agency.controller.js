...
const getAgency = async (req, res) => {
    try {
        ...
        const agency = await (0, agency_service_1.getAgencyById)(agencyId);
        if (!agency) {
...
const editAgency = async (req, res) => {
    try {
        const agencyId = String(req.params.id);
        const allowed = await canManageAgency(req.user.userId, req.user.role, agencyId);
        if (!allowed) {
        ...
        const agency = await (0, agency_service_1.updateAgency)(agencyId, validatedData);
        return res.status(200).json({
            success: true,
...
const listBranches = async (req, res) => {
    try {
        ...
        const branches = await (0, agency_service_1.getAgencyBranches)(agencyId);
        return res.status(200).json({
            success: true,
            data: branches,
        });
...
const addBranch = async (req, res) => {
    try {
        ...
        const branch = await (0, agency_service_1.createAgencyBranch)(agencyId, validatedData);
        return res.status(201).json({
            success: true,
...
const editBranch = async (req, res) => {
    try {
        ...
        const branch = await (0, agency_service_1.getBranchById)(branchId);
        if (!branch) {
            return res.status(404).json({
                success: false,
        ...
        const updatedBranch = await (0, agency_service_1.updateAgencyBranch)(branchId, validatedData);
        return res.status(200).json({
            success: true,
