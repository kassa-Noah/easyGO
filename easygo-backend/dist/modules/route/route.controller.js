...
const getRoute = async (req, res) => {
    try {
        const route = await (0, route_service_1.getRouteById)(req.params.id);
        if (!route) {
...
const editRoute = async (req, res) => {
    try {
        const existingRoute = await (0, route_service_1.getRouteById)(req.params.id);
        if (!existingRoute) {
            return res.status(404).json({
                success: false,
                message: "Route not found",
        ...
        const route = await (0, route_service_1.updateRoute)(req.params.id, validatedData);
        return res.status(200).json({
            success: true,
