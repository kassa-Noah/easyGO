...
const getTrip = async (req, res) => {
    try {
        const trip = await (0, trip_service_1.getTripById)(req.params.id);
        if (!trip) {
...
const editTrip = async (req, res) => {
    try {
        const existingTrip = await (0, trip_service_1.getTripById)(req.params.id);
        if (!existingTrip) {
            return res.status(404).json({
                success: false,
                message: "Trip not found",
        ...
        const trip = await (0, trip_service_1.updateTrip)(req.params.id, validatedData);
        return res.status(200).json({
            success: true,
