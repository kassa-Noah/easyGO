...
class ManageBookingStatusScreen extends StatefulWidget {
  const ManageBookingStatusScreen({super.key, required this.booking});
...
class _ManageBookingStatusScreenState extends State<ManageBookingStatusScreen> {
  ...
  void initState() {
    ...
    _currentStatus = widget.booking['status'] as String;
  }
  ...
  bool get _canManage => _allowedTransitions.isNotEmpty;
  ...
  String _transitionDescription(String status) {
    switch (status) {
      case 'Completed':
  ...
  Future<void> _updateStatus() async {
    if (_selectedStatus == null) {
      _showMessage('Select the new booking status.');
      return;
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Manage Booking Status')),
      body: Container(
              ...
              child: ConstrainedBox(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 18),
                    if (_canManage)
                      _buildTransitionSection(context)
                    else
                      _buildFinalStatusNotice(context),
                    const SizedBox(height: 18),
        ...
        children: [
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text(
                  widget.booking['bookingReference'] as String,
                ...
                Text(
                  widget.booking['clientName'] as String,
  ...
  Widget _buildTransitionSection(BuildContext context) {
    return GlassContainer(
      width: double.infinity,
      padding: const EdgeInsets.all(18),
      borderRadius: 18,
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 18),
          ..._allowedTransitions.map((status) {
            return Padding(
              padding: const EdgeInsets.only(bottom: 12),
              child: _StatusOptionCard(
                status: status,
                description: _transitionDescription(status),
  ...
  Widget _buildFinalStatusNotice(BuildContext context) {
    return GlassContainer(
      width: double.infinity,
      padding: const EdgeInsets.all(18),
      borderRadius: 18,
  ...
  Widget _buildUpdateButton() {
    return SizedBox(
      ...
      child: FilledButton.icon(
        ...
        label: Text(
          _isSaving ? 'Validating Status...' : 'Update Booking Status',
        ),
      ),
    );
  }
