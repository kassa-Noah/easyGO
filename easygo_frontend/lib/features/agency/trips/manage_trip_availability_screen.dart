...
class _ManageTripAvailabilityScreenState
  ...
  void initState() {
    ...
    _currentCapacity = widget.trip['totalSeats'] as int;

    _bookedSeats = widget.trip['bookedSeats'] as int;

    _capacityController = TextEditingController(
      text: _currentCapacity.toString(),
    );

    _capacityController.addListener(_refreshPreview);
  }

  @override
  void dispose() {
    _capacityController.removeListener(_refreshPreview);
  ...
  int get _enteredCapacity {
    return int.tryParse(_capacityController.text.trim()) ?? 0;
  }

  int get _previewAvailableSeats {
    final int capacity = _enteredCapacity;
  ...
  double get _previewOccupancy {
    ...
    final double value = _bookedSeats / capacity;
  ...
  Future<void> _saveAvailability() async {
    ...
    final int newCapacity = int.parse(_capacityController.text.trim());

    if (newCapacity == _currentCapacity) {
      _showMessage('No capacity change has been made.');
      return;
    }
  ...
  void _resetCapacity() {
    _capacityController.text = _currentCapacity.toString();
  ...
  void _decreaseCapacity() {
    ...
    if (current <= _bookedSeats) {
      _showMessage('Capacity cannot be lower than the number of booked seats.');
      return;
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Trip Availability')),
      body: Container(
                ...
                child: Form(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _buildCurrentAvailability(context),
                      const SizedBox(height: 18),
                      _buildCapacityEditor(context),
                      const SizedBox(height: 18),
  ...
  Widget _buildCurrentAvailability(BuildContext context) {
    final int availableSeats = _currentCapacity - _bookedSeats;

    final double occupancy = _currentCapacity == 0
        ? 0
        : _bookedSeats / _currentCapacity;
    ...
    return GlassContainer(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _sectionTitle(context, 'Current Availability'),
          const SizedBox(height: 18),
          Row(
            children: [
              Expanded(
  ...
  Widget _buildCapacityEditor(BuildContext context) {
    return GlassContainer(
      width: double.infinity,
      padding: const EdgeInsets.all(18),
      borderRadius: 18,
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _sectionTitle(context, 'Update Capacity'),
  ...
  Widget _buildPreview(BuildContext context) {
    ...
    final bool valid =
        capacity >= _bookedSeats && capacity > 0 && capacity <= 100;
    ...
    return GlassContainer(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Expanded(child: _sectionTitle(context, 'Availability Preview')),
              Icon(
          ...
          Row(
            children: [
              ...
              Expanded(
                child: _PreviewItem(
                  label: 'Available',
                  value: valid ? '$_previewAvailableSeats' : '—',
                ),
              ),
            ],
          ),
          ...
          Row(
            children: [
              ...
              Text(
                valid ? '${(_previewOccupancy * 100).round()}%' : '—',
                style: Theme.of(
                  context,
                ).textTheme.bodySmall?.copyWith(fontWeight: FontWeight.w600),
              ),
          ...
          ClipRRect(
            borderRadius: BorderRadius.circular(20),
            child: LinearProgressIndicator(
              value: valid ? _previewOccupancy : 0,
              minHeight: 7,
              backgroundColor: AppColors.primary.withValues(alpha: 0.10),
            ),
          ),
      ...
      children: [
        SizedBox(
          ...
          child: FilledButton.icon(
            ...
            label: Text(
              _isSaving ? 'Validating Availability...' : 'Update Availability',
            ),
          ),
        ),
        ...
        SizedBox(
          ...
          child: OutlinedButton.icon(
            onPressed: _isSaving ? null : _resetCapacity,
...
class _AvailabilityValue extends StatelessWidget {
  const _AvailabilityValue({required this.value, required this.label});
